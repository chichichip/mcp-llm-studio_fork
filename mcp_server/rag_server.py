...
결정론적으로 해야 한다(저장소 CLAUDE.md 'Vision RAG' 절). 인덱스에는 성격이 다른
문서(라우팅 지침서 / 설계기준 / 치수표 / 스펙)가 섞여 있고 쓰는 법이 다르므로,
`doc_roles`가 역할을 판정해 **응답에 코드가 직접 안내를 박아 넣는다** — 치수표의
숫자는 검증 전이고, 설계기준의 식은 계산 과정을 보여야 한다.
...
# ─────────────────────────── 문서 역할 안내 ───────────────────────────
# 같은 인덱스에 성격이 다른 문서가 섞여 있고 **쓰는 법이 다르다**(doc_roles.py 참고):
# 치수표의 숫자는 검증 없이 쓰면 안 되고, 설계기준의 식은 계산 과정을 보여야 하며,
# 사내 지침서는 대개 다음 문서를 가리키기만 한다. 그 구분을 응답에 코드가 직접 박는다
# — 프롬프트로 부탁하면 모델이 요약하며 떨어뜨린다(standard_part_server와 같은 방침).
#
# 예전에는 `STD_SPEC_DIR` **폴더 안**인지만 봤다. 그러면 AS568 같은 공개 표준 치수표가
# 일반 폴더에 있을 때 아무 경고 없이 숫자가 흘러나온다 — 실제로 그렇게 새어 나갔다.


def _role(store, path: str) -> tuple[str, str]:
    """이 문서의 역할. 판정에 쓸 전사문 표본은 인덱스에서 꺼낸다(원본을 열지 않는다)."""
    cached = doc_roles.role_of(path)          # 캐시에 있으면 본문을 안 읽는다
    if cached[0]:
        return cached
    try:
        sample = store.file_sample(path)
    except Exception:  # noqa: BLE001 — 역할 판정 실패가 검색을 막으면 안 된다
        sample = ""
    return doc_roles.role_of(path, sample)
...
def search_docs(query: str, top_k: int = 5) -> str:
    ...
    for _cid, c in order:
        ...
        i += 1
        role, _why = _role(store, c["path"])
        roles.append(role)
        tag = f"  [{doc_roles.role_name(role)}]" if role else ""
        out.append(f"[{i}] {_cite(c)}" + tag)
        out.append(content)
        out.append("")
    note = doc_roles.note_for(roles)
    if note:
        out.append(note)
    return "\n".join(out).rstrip()
...
def read_page(document: str, page: int) -> str:
    ...
    out.append("")
    out.append(body)
    # 한 쪽을 통째로 돌려주는 자리라 치수표가 그대로 딸려 나온다 — search_docs보다
    # 오히려 '표를 다 읽었다'고 착각하기 쉽다. 여기서도 역할 안내를 붙인다.
    note = doc_roles.note_for([_role(store, path)[0]])
    if note:
        out.append("")
        out.append(note)
    return "\n".join(out)
...
def ask_page(document: str, page: int, question: str) -> str:
    ...
    # VLM으로 다시 읽은 값은 가장 그럴듯해 보이는데 검증은 가장 덜 된 값이다.
    note = doc_roles.note_for([_role(store, path)[0]])
    if note:
        out += "\n\n" + note
    return out
