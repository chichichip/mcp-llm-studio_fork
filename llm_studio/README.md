...
### 도구 스키마 크기 (컨텍스트가 빠듯한 게이트웨이용)

도구 스키마는 **매 요청마다 통째로** 다시 나가는 고정 비용이다. 서버 넷을 다 켜면:

| 상황 | 도구 | ≈토큰 |
|---|---|---|
| 다이어트 없음, 전체 노출 | 67 | 12,900 |
| `tool_schema_slim`(기본 켬) | 67 | 11,200 |
| 스코프 적중("엑셀…" → office만) | 30 | 5,100 |
| `tool_scope_fallback=["office","docs"]` | 32 | 5,300 |

- `tool_schema_slim` — 도구 설명을 **첫 문단만** 남긴다. 뒤쪽 상세는 도구를 *고르는* 데는
  필요 없고, 정작 필요할 땐 도구 응답과 [작업 중인 문서] 블록이 같은 얘기를 해 준다.
  이름·`required`·`type`·`default`·`enum`은 절대 건드리지 않는다.
- 다만 **설명은 전체의 32%뿐**이고 나머지 64%가 JSON 구조(괄호·키·타입)라, 다이어트만으로는
  13%밖에 못 줄인다. **진짜 지렛대는 도구 개수**다.
- `tool_scope_fallback` — 어느 서버인지 못 정한 질문에서 전체 대신 이 목록만 노출한다.
  비우면(기본) 전체. 컨텍스트가 빠듯하면 자주 쓰는 서버만 적어 매 턴 절반 이하로 줄인다.
  ⚠ 목록 밖 서버는 질문에 그 낱말이 없으면 안 보인다 — "메일 보내줘"처럼 낱말이 걸리면
  그때는 정상적으로 노출된다.
- 안 쓰는 서버는 아예 `disabled`로 끄는 게 가장 확실하다(outlook 하나가 32개·6,200토큰).
