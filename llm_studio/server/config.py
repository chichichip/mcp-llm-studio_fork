...
DEFAULT_CONFIG = {
    ...
    # 어느 서버인지 못 정했을 때 노출할 '기본 세트'. 비우면 전체를 보여준다(안전한 기본값).
    # 도구를 다 보내면 컨텍스트가 터지는 게이트웨이에서는 여기에 자주 쓰는 서버만 적는다
    # (예: ["office", "docs"]). 대신 목록 밖 서버는 그 낱말이 질문에 없으면 안 보인다.
    "tool_scope_fallback": [],
    # ── 도구 스키마 다이어트 ──
    # 도구 스키마는 매 요청마다 통째로 다시 나가는 고정 비용이다(서버 넷 = 도구 67개 =
    # 4만 자에 육박). 컨텍스트가 빠듯한 게이트웨이에서는 이것만으로 한도를 넘긴다.
    # 도구 설명은 첫 문단만 남긴다 — 뒤쪽 상세는 도구를 '고르는' 데는 필요 없고, 정작
    # 필요한 시점에는 도구 응답과 [작업 중인 문서] 블록이 같은 얘기를 해 준다.
    # ⚠ 너무 줄이면 약한 모델의 도구 선택이 나빠진다. 도구를 못 고르기 시작하면 늘릴 것.
    "tool_schema_slim": True,
    "tool_schema_desc_chars": 200,   # 도구 설명 상한(0이면 설명 없음 — 권장하지 않음)
    "tool_schema_arg_chars": 80,     # 인자 설명 상한
...
# ⚠ 여기 **일반어를 넣지 말 것.** 거의 모든 질문이 한 서버에 걸려 나머지 도구가
#   사라진다. 실제로 그렇게 깨졌다: `치수`가 catia 낱말이라 오링 홈 치수 질문이
#   catia로 걸렸고, `std`는 낱말이 아예 없어 **`select_dash`가 도구 목록에서 통째로
#   사라졌다.** 모델이 안 부른 게 아니라 못 부른 것인데, 화면에는 "그 도구가 없다"
#   로만 보여 원인을 찾기 어렵다. 그래서 `수식`·`치수`·`형상`·`해석`을 뺐다.
#
# ⚠ **번들 서버는 전부 여기에 항목이 있어야 한다.** 빠지면 그 서버는 키워드로는
#   영영 안 걸려, 다른 서버가 하나라도 걸리는 순간 조용히 사라진다.
#   `check_repo.py`가 이 대응을 검사한다.
DEFAULT_TOOL_SCOPE_KEYWORDS = {
    ...
    "outlook": ["메일", "이메일", "e-mail", "mail", "outlook", "아웃룩", "받은편지함",
                ...
                "초안", "발송", "참조자"],
    # 부품 선정 낱말(규격·선정·오링·AS568…)을 std와 **겹쳐서** 둔다. 이 도메인에서
    # 둘은 한 쌍이다 — 지침서·설계기준에서 식과 규격을 찾고(docs) 그 표에서 부품번호를
    # 확정한다(std). 한쪽만 걸려 사슬이 끊기면 모델이 근거 없이 숫자를 주워 온다.
    "docs": ["규정", "지침", "매뉴얼", "사규", "사내문서", "사내 문서", "rag",
             "규격", "설계기준", "선정", "오링", "o-ring", "arp", "as568",
             "압축률", "스퀴즈", "squeeze"],
    # 표준품 선정은 **docs와 한 쌍으로 움직인다** — 지침서에서 규격을 찾고(docs)
    # 그 표에서 부품번호를 확정한다(std). 그래서 지침서 쪽 낱말(규격·치수·선정)을
    # 여기에도 둔다. 한쪽만 걸려 사슬이 끊기면 모델이 숫자를 주워와 답한다.
    "std": ["표준품", "부품번호", "품번", "dash", "대시", "규격", "치수", "선정",
            "오링", "o-ring", "오 링", "볼트", "너트", "와셔", "워셔", "클램프",
            "체결", "파스너", "fastener", "as568", "arp", "ms9", "nas", "도면번호"],
    "intranet": ["포털", "사내망", "인트라넷", "sharepoint", "쉐어포인트", "그룹웨어",
                 "게시판", "공지", "사내 사이트", "사내사이트"],
    "pdf": ["pdf", "피디에프"],
    "catia": ["catia", "카티아", "스케치", "어셈블리", "모델링", "구속조건"],
    "ansys": ["ansys", "앤시스", "mapdl", "열해석", "메시", "시뮬레이션", "경계조건"],
}
