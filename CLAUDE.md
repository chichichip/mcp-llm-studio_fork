...
### `llm_studio/vlm_read.py` — 스캔 규격서 VLM 판독 CLI

텍스트 층이 없는 스캔 PDF(또는 이미지)를 사내 VLM(OpenAI 호환, 예: muse-glimmer)으로 읽어 표를 마크다운으로 옮긴다. venv를 활성화한 터미널에서 `python vlm_read.py <PDF> --pages 2~12 --rotate 6~:90 --url <사내>/v1 --key <키>`처럼 쓴다. 의존성은 pymupdf(사내 미러 확인됨) + 표준 라이브러리. **런타임 HTTP가 localhost가 아닌 유일한 예외**다 — 사용자가 `--url`로 지정한 사내 VLM 서버 한 곳만 부르고, 인터넷은 쓰지 않는다.

- **원본 + 타일을 한 요청에**: VLM 서버가 이미지를 줄여 보기 때문에 촘촘한 표 숫자가 뭉개진다. 페이지마다 저해상도 원본(구조 파악용) + 300dpi 타일(숫자 판독용)을 함께 보내고, 페이지 단위 요청을 병렬로 보낸다(`--workers`). 기본은 **가로 띠**(`--split rows`) — 격자는 표의 한 행을 좌우로 갈라 행 맞춤을 모델에게 떠넘긴다. 타일은 겹친다(`--overlap`).
- **회전은 수동 지정**(`--rotate 6~:90`, 값 = 시계방향 각도, `prerotate(+90)`이 시계방향임을 실측 확인). 가로 표를 눕혀 넣고 머리글만 똑바로 인쇄한 스캔 페이지는 VLM이 똑바른 머리글에 속아 방향을 오판하고, 스캔본이라 텍스트 방향으로 자동 판정할 수도 없다. 회전을 **먼저** 적용한 뒤 타일 영역을 회전 후 좌표로 정하고 역행렬로 clip을 만든다 — 띠가 돌린 뒤의 행을 따라 잘리게 하려는 것이니 이 순서를 바꾸지 말 것.
- **쪽 지정은 PDF 뷰어 기준**(`2~12`, `2~5,8`, `6~`). 잘못된 지정은 요청을 하나도 보내기 전에 막는다.
- **출력**(`vlm_out/<파일>_<시각>/`, 실행마다 새 폴더, gitignore): `all.md`(합본 — 첫 줄 `[추출본: 문서 p.N]`이라 Nemotron에 그대로 붙여넣는다), `pages/`, `images/`(실제로 보낸 이미지 — 오독이 VLM 탓인지 자르기 탓인지 가리는 증거), `run.json`(**키는 저장하지 않는다**), `diff.md`(`--runs 2` — 두 번 읽어 다른 줄만). 실패한 쪽은 빼지 않고 `[추출 실패]` 자리표시를 남기고, `max_tokens`로 잘린 출력은 눈에 보이는 경고를 붙인다 — 표가 조용히 빠지는 걸 막는 장치들이니 없애지 말 것.
- `--dry-run`은 VLM을 부르지 않고 이미지와 픽셀 크기만 낸다. ⚠ 실기 검증 대상: 사내 VLM 서버가 이미지를 **긴 변 기준**으로 줄이면 폭이 넓은 가로 띠(회전 페이지에서 3500px대)는 원본과 해상도가 비슷해져 이득이 줄어든다. 그 경우 `--split grid --tiles 2`와 비교해 볼 것.
