...
- **임베딩은 llama-server `--embeddings`**(기본 `http://127.0.0.1:8001/v1`, EmbeddingGemma 등 GGUF를 CPU `-ngl 0` 상주 권장). 띄우는 건 `run_embed_server.bat` — **경로를 bat에 박지 않고** `local_settings.py`의 `RAG_EMBED_GGUF`/`RAG_LLAMA_SERVER`에서 읽는다(공개 저장소라 사내 경로를 못 넣고, 넣어도 저장소를 다시 받으면 날아간다). bat이 `python settings.py embed-launch`를 한 번 불러 `set` 할 줄들을 받아 간다. **포트는 `RAG_EMBED_URL`에서 파생시킨다** — 서버를 8001에 띄웠는데 검색이 다른 포트를 보면 오류 없이 조용히 키워드 전용으로 저하해 원인 찾기가 어렵다. 모델을 못 찾을 때 아무 `.gguf`나 집지 않는다(`*embed*`/`*bge*`/`*e5*`만) — 채팅 모델을 임베딩 서버로 띄우면 차원이 안 맞아 인덱스가 통째로 어긋난다. **서버가 없으면 키워드 인덱스만 만들고 검색도 키워드 전용으로 우아하게 저하** — 나중에 서버를 켜고 `--reindex`로 돌리면 벡터가 붙는다. 이 저하 경로 덕에 임베딩 모델 반입 전에도 개발·검증이 가능하다.
...
- **회전 판정은 반드시 쪽 하나씩 묻는다** (`detect_rotation(doc, [pno])`). 여러 쪽을 한꺼번에 넘기면 **표지·목차의 가로쓰기가 누운 치수표를 표결에서 이겨** 0도가 나온다 — `--pages 3`으로 한 쪽만 주면 읽히는데 찾기 창(1~6쪽)으로 주면 못 읽는, 원인이 화면에 전혀 안 보이는 실패가 실제로 났다. 판정은 텍스트 레이어만 보므로 쪽마다 해도 VLM 비용이 0이다. 인제스트·`ensure_page_image`·치수표 판독 모두 `[pno]` 하나씩 넘긴다.
- **"일부라도 읽혔으면 문서는 바로 선 것"이라고 보지 말 것.** 누운 문서에서 **어쩌다 한 쪽이 0도로 읽히면** 그 한 쪽이 재시도를 통째로 막아 나머지 표 쪽이 전부 0행인 채 끝난다(위 실패의 나머지 절반이다). 그래서 재시도는 전멸했을 때가 아니라 **못 읽은 쪽이 있을 때** 돌린다 — 표가 나온 쪽 바로 옆의 못 읽은 쪽을 골라 다른 각도를 시험하고, 통하면 그 각도로 못 읽은 쪽을 한꺼번에 되살린다(`learned_rot`). 다만 **글자 방향을 이미 아는 쪽은 건드리지 않는다**(`text_dir_known`) — 방향을 아는 쪽이 0행이면 회전 문제가 아니라 그냥 표가 없는 쪽(표지)이고, 거기에 각도를 더듬으면 쪽당 수십 초를 그냥 버린다. 그래서 텍스트 레이어가 있는 문서는 **쪽당 VLM 호출이 정확히 한 번**이고, 스캔본만 문서당 세 번까지 더 쓴다.
- **누운 페이지는 돌려서 렌더링한다** (`detect_rotation`). 도면·치수표는 `/Rotate` 없이 **내용만 옆으로 그려진** PDF가 흔하다(CAD 출력·스캔본). PyMuPDF는 `/Rotate`만 적용하므로 그런 쪽은 누운 채 렌더되고, VLM은 오류가 아니라 **빈 결과**를 돌려준다 — 화면만 봐서는 원인이 안 보인다(AS568 치수표가 실제로 그랬다: "치수표를 찾지 못했습니다"만 나왔다). 판정은 텍스트 레이어의 글자 방향(`dir`)으로 **VLM 없이 공짜로** 하고, 가로쓰기가 섞여 있으면 누운 쪽이 확실히 우세할 때만 돌린다(바로 선 쪽을 잘못 돌리면 멀쩡하던 전사가 통째로 깨진다). 스캔본은 텍스트 레이어가 없어 이 판정이 0을 주므로, **호출부가 0행일 때 각도를 바꿔 다시 부르는** 쪽으로 저하한다(`spec_table.read_table`). 인제스트·`ensure_page_image`·치수표 판독이 **같은 판정을 쓴다** — 한쪽만 돌리면 캐시된 그림과 즉석 렌더가 달라져 `ask_page`가 딴 그림을 본다.
...
**인덱스에는 성격이 다른 문서 네 종류가 섞이고, 쓰는 법이 다르다** (`doc_roles.py`). 실측으로 확인된 구조다:

| 역할 | 무엇 | 쓰는 법 |
|---|---|---|
| `routing` | 사내 지침서 | **라우팅 표**다 — "이 요구사항이면 저 규격/도면을 봐라". 전 계열이 이 모양이다(오링은 규격번호+규격명, 클램프·볼트는 도면번호+품명) |
| `design` | 규격 설계기준(ARP1231 등) | **계산식이 여기 있다.** 오링처럼 설계기준이 따로 있는 계열도 있고 없는 계열도 있다 |
| `table` | 치수표(AS568 등) | 🔴 dash별 숫자. **RAG로 읽으면 안 된다** — `select_dash`로 |
| `spec` | 규격 본문 | 재질·온도·적용 범위 |

**일반화되는 것은 선정 규칙이 아니라 "지침서에서 다음 문서로 가는 한 번의 hop"이다.** 계열마다 선정 기준이 다른 건 맞지만(오링은 홈, 볼트는 그립), 지침서가 그걸 표로 라우팅해 주는 형식은 전 계열 공통이다. 그래서 `selection.py`에 계열별 규칙을 하드코딩하는 것보다 **설계기준 문서를 읽게 두는 쪽이 낫다** — 실제로 모델이 `ARP1231`의 압축률 식을 원문 그대로 인용했다(실측). 대신 `design` 문서에는 "인용한 식·대입한 숫자·결과를 모두 보이라"는 안내를 코드가 붙인다.

역할은 **파일명 힌트 + 전사문 내용**으로 자동 판정한다(문서가 많아 사람이 일일이 못 적는다). 파일명만으로는 `ARP1231.pdf`(스펙)와 `ARP1231_O-RING GENERAL DESIGN.pdf`(설계기준)를 가르기 어렵고, 내용만으로는 짧은 문서를 못 가른다. **애매하면 `table` 쪽으로 기운다** — 경고를 놓치는 것이 한 번 더 붙는 것보다 나쁘다. 자동 판정이 틀리면 `local_settings.py`의 `RAG_DOC_ROLES`(파일명 패턴 → 역할)로 덮어쓴다; 사람이 적은 것이 언제나 이긴다. ⚠ `STD_SPEC_DIR` 아래라는 사실은 **자동 판정보다 뒤에** 둔다(달리 못 가린 파일만 `table`로) — 그 설정을 지침서·설계기준이 섞인 폴더로 잡는 운용이 실제로 있고(스펙 판독은 `spec_dir` 인자로도 넘길 수 있다), 앞에 두면 그 폴더의 모든 문서가 '치수표'가 돼 경고가 사방에 붙는다. **경고가 흔해지면 무시당한다.**

`rag_server`의 `search_docs`·`read_page`·`ask_page` 세 경로 모두가 결과에 **역할 딱지**를 붙이고 역할별 안내를 **코드가 직접 박아 넣는다**(프롬프트로 부탁하면 모델이 요약하며 떨어뜨린다). ⚠ 예전에는 `STD_SPEC_DIR` **폴더 안**인지만 봤는데, 그러면 AS568 같은 공개 표준 치수표가 일반 폴더에 있을 때 아무 경고 없이 숫자가 흘러나갔다 — 실제로 그렇게 샜다. `standard_part_server`도 근거를 전부 "지침서"라고 적던 것을 실제 역할로 바꿨다: **국제 규격 조항이 사내 규정처럼 보이면 안 된다.**
...
"무엇이 필요한가"를 받아 **어느 계열(중분류·도면번호)을 보면 되는지**까지 좁히는 읽기 전용(🟢) 서버. 도구 6개: `find_standard`(요구사항 → 계열 후보 + 지침서 근거) / `list_parts`(계열의 부품번호 조회) / `read_spec_table`(스펙 도면 치수표 판독) / `select_dash`(조건으로 부품번호 확정) / `find_mating_part`(짝 너트) / `std_status`(진단). http/sse는 :8094. llm_studio에는 `std`로 등록되지만 **기본 `disabled`** — `STD_CATALOG_PATH`를 설정하기 전에는 도구만 늘기 때문이다.
...
- **부품번호 확정은 `spec_table.py`가 한다** — 도면을 RAG에 넣지 않는다. 치수표를 RAG로 검색하면 청크 경계에서 행이 잘리고 어느 dash의 값인지 흐려진다. 도면은 검색할 필요도 없다: 엑셀의 **도면번호/품명이 곧 파일 이름**이라 `find_spec_file`이 둘 다로 찾는다. 표는 통째로 판독해 캐시하고(VLM은 쪽당 수 초), `verify.py`로 검증한 뒤 조건으로 거른다.
- **선정에는 두 갈래가 있고 지금은 앞의 것만 된다**: *직접 조건*("나사 .190-32, 머리 ≤0.35")은 표의 컬럼 비교라 **계열마다 규칙이 필요 없다** — `spec_table.match`가 어느 표에나 쓰인다. *간접 조건*("체결두께 12" → 그립 → dash)은 계열별 규칙이 필요해 지침서 조항이 확정되면 `selection.py`에 붙인다.
- ⚠ **치수 기호의 뜻은 모델이 추측하면 안 된다.** 표 헤더에는 알파벳(H, K, L…)만 있고 그게 무슨 치수인지는 **도면 그림의 지시선에만** 있다. 'H니까 Height겠지'로 찍으면 엉뚱한 컬럼으로 걸러 잘못된 부품이 나오고 아무도 못 알아챈다. 그래서 `LEGEND_PROMPT`가 **그림을 읽어** 기호별 뜻을 판독하고(한국어 설명 포함), `read_spec_table`이 그 범례를 함께 보여준다. `select_dash`는 조건의 키를 `resolve_columns`로 확정하는데 — **확정 못 하거나 여러 기호에 걸리면 거르지 않고 거절한다.** 조건이 무시된 채 '전부 통과'로 보이는 게 가장 위험하기 때문이다. 범례도 VLM 판독이라 검증되지 않았음을 응답에 명시한다.
- 조건 표기는 **`a..b`(구간)**/`<=`/`>=`/`<`/`>`/`~`(셀 범위가 값을 포함)/숫자/나사규격/문자열. **치수표에는 구간을 쓴다** — 표준품 치수는 이산값이라 '정확히 3.1mm'인 행이 대개 없고, 점으로 물으면 늘 0건이 나와 도구가 쓸모없어진다. `a..b`와 `~x`는 **방향이 반대**다(전자는 조건이 범위·셀이 점, 후자는 조건이 점·셀이 범위). 셀이 공차 범위면 구간과 **겹치기만 해도** 통과시킨다 — 공차 안에 답이 있을 수 있는 행을 미리 지우면 안 된다. 같은 치수가 단위별로 컬럼이 나뉘어 있으면(`I.D. INCHES`/`I.D. MILLIMETERS`) 단위까지 붙여야 한다 — 안 그러면 `resolve_columns`가 둘 다에 걸려 거절한다. 셀이 범위면 `<=`는 **최대값**으로, `>=`는 최소값으로 본다 — 최악값 기준이 안전한 쪽이다. **표에 없는 컬럼을 조건으로 주면 조용히 무시하지 않고 알린다**(조건이 안 걸린 채 '전부 통과'로 보이면 잘못된 부품을 고른다).
- ⚠ `spec_table`은 **`read_spec.py`를 import하지 않는다** — 그 파일은 `config.py`가 없으면 import 시점에 `sys.exit()`으로 프로세스를 죽여 MCP 서버가 통째로 내려간다. 순수 로직(`prompts`/`merge`/`verify`)만 쓰고 VLM은 `vision_ingest`(local_settings로 이미 설정됨)를 재사용한다.
- **판독은 쪽마다 캐시에 적는다** (`_page_cache_path`, `<도면>_<해시>_pages.json`). 사내 게이트웨이는 쪽당 수십 초라 20쪽이면 10분이 넘는데, 끝에 한 번만 저장하면 **Ctrl+C 하거나 창을 닫는 순간 그 시간이 통째로 날아가고 다시 돌려도 같은 자리에서 또 기다린다** — 실제로 그렇게 잃었다. 쪽마다 적어 두면 어디서 끊기든 다음 실행이 이어간다(회전각별로 담아 각도를 바꿔 재시도해도 앞서 읽은 것을 안 버린다). 한 쪽만 오독했으면 `--pages 3 --refresh`로 **그 쪽만** 다시 읽어 고친다. `STD_SPEC_TIME_BUDGET`(기본 1800초)을 넘으면 읽은 데까지 저장하고 멈춘다 — 끝이 언제인지 모르는 채 기다리는 것보다 낫다. Ctrl+C도 예외로 죽지 않고 **가진 것으로 결과를 만들어** 돌려준다(10분 기다린 사람에게 빈손은 안 된다). ⚠ **끝까지 못 읽은 결과는 최종 캐시(`_save_cache`)에 넣지 않는다** — 넣으면 다음 호출이 '이미 다 읽었다'고 보고 모자란 표를 그대로 쓴다. `complete: False`가 붙고 `read_spec_table`이 그 사실을 응답에 박아 넣는다(모자란 표로 부품을 고르면 아무도 못 알아챈다).
- **치수표는 찾은 뒤 끊길 때까지 이어 읽는다.** 찾기 창은 `STD_SPEC_MAX_PAGES`(기본 6쪽)인데, 체결구 도면은 표가 한두 쪽이지만 AS568 같은 치수 목록은 4쪽부터 수십 쪽까지 이어진다. 찾기 창에서 끊으면 표의 앞부분만 읽고 조용히 멈춘다 — **행이 모자란 걸 아무도 못 알아챈다**. 그래서 표가 나온 뒤에는 0행이 나오는 쪽까지 이어 읽고(`STD_SPEC_MAX_TABLE_PAGES`, 기본 40쪽 상한), 상한에 걸리면 알림을 남긴다. CLI `--pages`는 `4-12` 구간 표기를 받는다.
- **JSON이 안 나오면 표만 옮겨 적게 하고 우리가 행으로 바꾼다** (`PLAIN_TABLE_PROMPT` → `_rows_from_markdown`). 실측: 같은 쪽을 `PAGE_PROMPT`(마크다운 전사)로 읽으면 30행이 멀쩡히 나오는데 `TABLE_PROMPT`(JSON)로 읽으면 0행이었다 — 이미지도 VLM도 멀쩡한데 **JSON을 요구하는 순간** 빈 결과가 온다(체결구 도면용 프롬프트라 AS568 같은 '목록 문서'를 보면 모델이 "그 표가 아니다"라고 판단해 버린다). JSON을 고집하다 표를 통째로 놓치는 것보다 **이미 되는 것으로 물러서는** 쪽이 낫고, 판단은 코드가 하고 VLM은 판독만 한다는 원칙에도 이쪽이 맞다. ⚠ **JSON이 제대로 와서 "행 없음"이라고 한 쪽은 다시 묻지 않는다** — 그건 모델이 약속한 형식으로 답하고 표가 없다고 말한 것(표지·목차)이라, 되물으면 쪽당 수십 초를 그냥 버린다. 되묻는 건 **형식을 안 지켰을 때**(산문 거절·잘림·호출 실패)뿐이다.
- **0행의 이유를 화면과 파일에 남긴다.** `ask_image_detail`이 `(응답, 사유)`를 돌려주고 `finish_reason == "length"`(응답 잘림)와 HTTP 코드별 해석을 구분한다 — 예전에는 "표가 없다"와 "응답이 잘렸다"와 "413"이 **전부 똑같이 0행으로만** 보여 고칠 곳을 못 찾았다. VLM 원문은 `<도면>_<해시>_raw/pNNN_<각도>_<json|plain>.txt`로 떨어진다(사내망은 로그를 반출할 수 없어 화면 밖에 남는 게 이것뿐이다). HTTP 안내가 무엇이 와도 "DPI를 낮추라"고 적던 것도 고쳤다 — 401·404에 그 말이 나오면 엉뚱한 곳을 뒤지게 된다.
- **`TABLE_PROMPT`는 spec-reader 것을 그대로 쓰고 보충만 덧붙인다**(`TABLE_PROMPT_EXTRA`). 원문은 체결구 도면(MS/AS/NAS)에 맞춰 **fixtures 회귀 시험이 걸린** 프롬프트라 건드리면 안 되는데, 이 저장소는 그 밖의 표도 읽는다 — AS568은 도면이 아니라 목록 문서이고, 컬럼이 기호(`H`,`K`)가 아니라 낱말(`I.D. MILLIMETERS`)이며 단위가 인치만이 아니다. 원문만 주면 "해당하는 표가 없다"로 0행이 나온다.
- ⚠ **치수표 판독은 앱의 도구 호출 제한(`CALL_TIMEOUT_SEC`, 기본 120초)을 넘길 수 있다.** 쪽마다 VLM을 부르는데 사내 게이트웨이는 쪽당 수십 초가 나온다. 타임아웃이 나면 캐시도 안 남아 **몇 번을 시도해도 같은 자리에서 실패한다.** 그래서 `spec_table.py`에 CLI를 뒀다 — `python mcp_server\spec_table.py AS568 --dir C:\rag\vision` 으로 **제한 밖에서 한 번 판독해 캐시에 넣으면** 이후 앱 호출은 즉시 끝난다(컬럼 이름도 여기서 확인한다). `MCP_CALL_TIMEOUT_SEC` 환경변수로 제한 자체를 늘릴 수도 있다. 그리고 `needs_legend()` — 컬럼이 `I.D. MILLIMETERS` 같은 **낱말이면 범례 VLM 호출을 아예 건너뛴다**(뜻이 이미 적혀 있다). 기호(`H`,`K`)가 있는 표에서만 그림을 읽는다.
- ⚠ **`@mcp.tool()` 데코레이터와 함수 사이에 다른 함수를 끼워 넣지 말 것.** 실제로 `select_dash` 위에 헬퍼를 넣었다가 데코레이터가 헬퍼를 감싸 **`select_dash`가 도구 목록에서 통째로 사라졌다** — 파이썬도 실행도 멀쩡해서 모델이 '그런 도구가 없다'고 할 때까지 모른다. `check_repo.py`가 비공개 이름(`_`)에 붙은 도구 데코레이터를 잡는다.
...
- **컨텍스트 관리** (`server/context.py`) — 이 앱은 매 턴 이력 전체를 보내는데 도구 결과 하나가 최대 2만 자(`TOOL_RESULT_MAX`)라, 두면 `ctx`가 차고 **앞쪽의 system 프롬프트가 밀려나 조용히 망가진다**. `age_messages`가 오래된 `role:tool` 메시지를 앞부분만 남기고 접는다 — **접는 건 모델에 보내는 사본뿐이고 `conv["messages"]` 원문은 절대 안 건드린다**(대화를 다시 열면 전부 있어야 하고, 접힌 자리의 안내를 보고 모델이 도구를 다시 부를 수 있어야 한다). `pick_tool_servers`는 질문·첨부로 서버를 좁히되 **애매하면 None(전체)을 돌려준다** — 잘못 좁혀 "못 한다"고 답하는 게 도구가 많은 것보다 나쁘다. 스코프 낱말(`DEFAULT_TOOL_SCOPE_KEYWORDS`)에 '문서'·'검색' 같은 일반어를 넣지 말 것: 거의 모든 질문이 한 서버에 걸려 나머지 도구가 사라진다. ⚠ **번들 서버는 전부 낱말 표에 항목이 있어야 한다** — 없으면 키워드로 영영 안 걸려, 다른 서버가 하나라도 걸리는 순간 조용히 사라진다. 실제로 `std`에 낱말이 없어 **`select_dash`가 모델에게서 통째로 사라졌고**, `치수`가 catia 낱말이라 오링 홈 치수 질문이 엉뚱하게 걸렸다 — 사람이 "그 도구를 쓰라"고 직접 명령해야 동작했다. 모델이 **안 부른 게 아니라 못 부른 것**인데 화면에는 "그런 도구가 없다"로만 보인다. `check_repo.py`가 이 대응을 검사하고 `llm_studio/test_tool_scope.py`가 실제 질문으로 확인한다. **`docs`와 `std`는 한 쌍이라 낱말을 겹쳐 둔다** — 지침서·설계기준에서 식을 찾고(docs) 그 표에서 부품번호를 확정한다(std). 한쪽만 걸려 사슬이 끊기면 모델이 근거 없이 숫자를 주워 온다. `slim_tools`는 도구 설명을 첫 문단만 남겨 스키마를 줄이되 이름·`required`·`type`·`default`·`enum`은 건드리지 않는다 — 이건 매 요청 고정 비용을 깎는 것이고(이력 접기는 과거를 깎는다), 컨텍스트가 빠듯한 사내 게이트웨이(vLLM 등) 대응이다. ⚠ 측정해 보면 **설명은 스키마의 32%뿐이고 64%가 JSON 구조**라 다이어트만으로는 13%밖에 안 줄어든다 — 도구 개수를 줄이는 게 유일한 큰 지렛대다(67개 12.9k토큰 → 스코프 적중 시 30개 5.1k토큰). 못 정했을 때 전체 대신 기본 세트만 쓰려면 `tool_scope_fallback`.
...
- **수식 렌더링(KaTeX)은 `static/vendor/katex/`에 파일로 반입돼 있다** — 폐쇄망이라 CDN을 쓸 수 없다. **폰트 20개를 CSS에 base64로 넣어 텍스트 파일 3개로 합쳤다**(`katex.min.js`·`auto-render.min.js`·`katex.min.css`, 합계 ~630KB): `.woff2` 같은 확장자가 사내 반입 검사에 걸릴 여지를 없애고, CSS의 `fonts/` 상대경로가 깨지는 사고도 막는다. 런타임에 네트워크로 나가지 않는 것은 확인했다(`fetch(`는 전부 KaTeX 파서의 토큰 선행읽기 메서드이고 `eval`·Worker·동적 script 주입 없음). `app.js`의 `typesetMath()`가 `md()`로 HTML을 만든 **뒤** 수식만 바꾸는데, **스트리밍 중에는 부르지 않는다** — 토큰마다 innerHTML이 다시 그려져 느리고 깜빡인다(스트림 끝과 대화를 다시 열 때만). `md()`는 `$$…$$`/`\[…\]`를 **한 요소 안에 묶는다**: 문단으로 쪼개지면 여는 쪽과 닫는 쪽이 다른 `<p>`에 들어가 KaTeX가 짝을 못 찾는다. `ignoredTags`에 `pre`/`code`가 있어 **도구 결과(치수표)는 건드리지 않는다**. KaTeX 파일이 없어도 채팅은 그대로 동작한다(수식만 원문으로 남는다).
...
`llm_studio/test_tool_scope.py` — 도구 스코프 회귀 시험. 앱도 MCP 서버도 없이 돌아 `python llm_studio\test_tool_scope.py`로 실행한다. 스코프 낱말을 고치면 돌려 볼 것.

`mcp_server/test_spec_table.py` — 치수표 판독 경로의 회귀 시험. **VLM도 PyMuPDF도 없이 돈다**(가짜 문서·가짜 VLM)이라 개발 PC에서 그냥 `python mcp_server\test_spec_table.py`로 실행한다. 여기서 잡는 실패는 **화면에 오류로 안 나타난다** — 회전을 잘못 잡거나 재시도 조건이 틀리면 VLM이 예외가 아니라 빈 결과를 돌려주고 그게 "0행"으로만 찍혀 표가 반쯤 읽힌 채 조용히 끝난다. 폐쇄망에서는 한 번 돌리는 데 수십 분이 걸려 시행착오로 알아낼 수가 없다. 판독 쪽 코드를 고치면 이걸 돌려 볼 것.

**커밋하기 전에 `python check_repo.py`를 돌린다** (표준 라이브러리만, 종료 코드로 실패를 알린다). 아래 규약 중 *글로 적어 두면 지켜질 것 같지만 조용히 깨지는* 것들을 기계가 확인한다 — bat의 CRLF+ASCII, `requirements.txt`의 UTF-8 BOM, 새 dotfile, 100MB 초과 파일, **허용 목록에 없는 호스트**(공개 저장소라 사내 주소가 커밋되면 안 된다), `@mcp.tool()` 함수 안의 stdout `print`(stdio 프로토콜 채널 오염), **스코프 낱말이 없는 번들 MCP 서버**(그 서버의 도구가 모델에게서 조용히 사라진다), **도구 응답 문자열 안의 마크다운**(llm_studio가 렌더링하지 않아 별표가 그대로 보인다). `--fix`가 줄바꿈·BOM은 고쳐 준다. 새 예시 주소를 쓰려면 `ALLOWED_HOSTS`에 사람이 추가해야 한다 — 그게 의도적으로 걸리적거리는 부분이다.
...
린트/테스트 커맨드는 그 밖에 따로 없다. 변경은 해당 스크립트를 직접 돌려서 검증한다.
