...
**스펙 도면도 RAG에 넣을 이유가 있다 — 단, 경고를 달아서.** 도면의 본문 글(REQUIREMENTS 문단의 재질 `AMS5731`·온도 `1200°F`·적용 범위)은 표가 아니라 문단이라 `spec_table.py`로는 못 읽는다. "A286 쓰는 볼트가 뭐 있나" 같은 질문은 RAG 말고 답할 길이 없고, 인덱싱하면 쪽 이미지가 캐시돼 `ask_page`로 그림을 되물을 수도 있다. 대신 검색이 **치수표 조각**도 돌려주게 되는데 그 숫자는 검증을 안 거쳤다 — `select_dash`에는 기호를 확정 못 하면 거절하는 방어가 있지만 **`search_docs`에는 없어서** 약한 모델이 그대로 답으로 내놓는 게 이 구조에서 가장 새기 쉬운 구멍이다. 그래서 `rag_server._is_spec_drawing`(경로가 `STD_SPEC_DIR` 아래인가)로 판별해 `search_docs`·`read_page`·`ask_page` 세 경로 모두에 **코드가 경고를 직접 박아 넣는다**(standard_part_server와 같은 방침 — 프롬프트로 부탁하면 모델이 요약하며 떨어뜨린다). `STD_SPEC_DIR`이 없으면 아무 일도 하지 않고, 도면이 결과에 안 섞이면 경고도 안 붙는다.
