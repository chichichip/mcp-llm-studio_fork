...
역할은 **파일명 힌트 + 전사문 내용**으로 자동 판정한다(문서가 많아 사람이 일일이 못 적는다). 파일명만으로는 `ARP1231.pdf`(스펙)와 `ARP1231_O-RING GENERAL DESIGN.pdf`(설계기준)를 가르기 어렵고, 내용만으로는 짧은 문서를 못 가른다. **애매하면 `table` 쪽으로 기운다** — 경고를 놓치는 것이 한 번 더 붙는 것보다 나쁘다. 자동 판정이 틀리면 `local_settings.py`의 `RAG_DOC_ROLES`(파일명 패턴 → 역할)로 덮어쓴다; 사람이 적은 것이 언제나 이긴다.
