...
- **실행 경로 두 가지 모두 유지한다**:
  ...
  - exe 배포 — `build_exe.bat`(PyInstaller) → `installer.iss`(Inno Setup) → `Setup.exe` 하나로 앱+llama-server 반입. 빌드는 개인 PC에서 한다. **`.spec` 파일은 저장소에 두지 않는다** — 파이썬 스크립트라 사내 반입 검사에서 막힌 적이 있어, PyInstaller 옵션을 전부 bat의 명령행 인자로 옮겼다(빌드 중 자동 생성되는 spec은 `.gitignore`). 옵션을 바꿀 땐 생성된 spec 말고 **bat을 고칠 것**. tkinter를 `--exclude-module`에 넣지 말 것 — 파일 선택 대화상자(`/api/fs/dialog`)가 함수 안에서 import한다(예전 spec이 이걸 제외해 exe에서 그 기능이 조용히 죽어 있었다).
